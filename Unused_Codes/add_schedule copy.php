...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && ($_GET['action'] ?? '') === 'get_schedules') {
  ...
  $stmt = $conn->prepare("SELECT shift, day, time, status FROM schedules");
  $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (in_array('', [$shift, $day, $time], true)) {
    json_response(['status' => 'error', 'message' => 'All fields are required.']);
  }

  $chk = $conn->prepare("SELECT 1 FROM schedules WHERE shift=? AND day=? AND time=? AND status IN ('Pending','Approved')");
  $chk->bind_param('sss', $shift, $day, $time);
  $chk->execute();
  $chk->store_result();
  if ($chk->num_rows > 0) {
  ...
  if ($model->addSchedule($user_id, $shift, $day, $time)) {
...
<body>
  ...
  <script>
    (function () {
      ...
      const fields = ['shift', 'day', 'time'];
      let selected = { shift: '', day: '', time: '' };
      let schedules = [];
      ...
      function showOptions(field) {
        ...
        const html = list.map(val => {
          const disabled = schedules.some(s =>
            ...
            s.time === (field === 'time' ? val : selected.time)
          );
