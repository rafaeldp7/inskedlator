  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <button class="btn btn-warning" id="editEventBtn">Edit</button>
...
<script>
  function initCalendarScripts() {
    ...
    $('.calendar-table')
      .off('click', 'td[data-day]')
      .on('click', 'td[data-day]', function () {
    ...
    $('#addEventForm')
      ...
      .on('submit', function (e) {
        ...
        const url = mode === 'edit' ? 'calendar_update_event.php' : 'calendar_add_event.php';
        const data = $(this).serializeArray();

        if (mode === 'edit') {
          data.push({ name: 'original_title', value: $(this).data('originalTitle') });
        }

        $.post(url, $.param(data), () => {
  document.activeElement.blur(); // Accessibility fix

  $('#addEventModal').modal('hide');

  // In case the backdrop doesn't go away, remove it manually after a short delay
  setTimeout(() => {
    $('body').removeClass('modal-open');
    $('.modal-backdrop').remove();
  }, 300); // Match Bootstrap's fade out timing

  loadCalendar(<?= $month ?>, <?= $year ?>);
});


        $(this).removeData('mode').removeData('originalTitle'); // reset mode
      });

    $('#eventModal').on('hidden.bs.modal', function () {
      $('#addEventBtn').focus(); // Fallback focus
    });
    ...
    $('#eventModal')
      ...
      .on('show.bs.modal', function (e) {
        ...
        const date = btn.closest('td').data('day');
        const formattedDate = `<?= $year ?>-<?= str_pad($month, 2, '0', STR_PAD_LEFT) ?>-${String(date).padStart(2, '0')}`;

        $('#modalEventTitle').text(title);
        $('#modalEventDesc').text(desc || 'No description available.');
        $('#modalEventDate').val(formattedDate);
        $('#editEventBtn').data({ title, desc, date: formattedDate });
        $('#deleteEventBtn').data({ title, date: formattedDate });
      });
    ...
    $('#editEventBtn').on('click', function () {
      ...
      $('#newEventDate').val(data.date);
      $('input[name="title"]').val(data.title);
      $('textarea[name="description"]').val(data.desc);
      $('#addEventForm').data('mode', 'edit').data('originalTitle', data.title);
    });

    $('#deleteEventBtn').on('click', function () {
      const data = $(this).data();
      if (confirm(`Delete event "${data.title}"?`)) {
        $.post('calendar_delete_event.php', data, function () {
          document.activeElement.blur(); // Accessibility fix
          $('#eventModal').modal('hide');
          loadCalendar(<?= $month ?>, <?= $year ?>);
        });
